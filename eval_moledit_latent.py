...
DEFAULT_CKPT_PATH = "/home/haoqian/Data/Molecule/Latent/checkpoints/stage1/last-v3.ckpt"
DEFAULT_OUTPUT_DIR = "/home/haoqian/Data/Molecule/Latent/eval_outputs/moledit"
...
def _pick_checkpoint(ckpt_path: str, ckpt_dir: str) -> str:
    ...
    cands = [os.path.join(ckpt_dir, n) for n in os.listdir(ckpt_dir) if n.endswith(".ckpt")]
    if not cands:
...
def _extract_predicted_smiles(text: str) -> Optional[str]:
    ...
    # Prefer longer candidates first.
...
def _load_model_and_tokenizer(args):
    ...
    return model, tokenizer, device, llama_version, eos_token_id, ckpt_path


def run_eval(args):
    model, tokenizer, device, llama_version, eos_token_id, ckpt_path = _load_model_and_tokenizer(args)
    ...
    for subtask, filename in MOLEDIT_TASK_FILES.items():
        ...
        for row in iterable:
            ...
            if source_smiles is None:
                # fallback: still try generation without graph.
                source_smiles = None
...
def main():
    parser = argparse.ArgumentParser(description="Inference + evaluation for MolEdit with Stage-I latent reasoning.")
    parser.add_argument("--train_config", default=DEFAULT_TRAIN_CONFIG)
    parser.add_argument("--checkpoint_path", default=DEFAULT_CKPT_PATH)
    parser.add_argument("--checkpoint_dir", default="/home/haoqian/Data/Molecule/Latent/checkpoints/stage1")
    ...
    parser.add_argument("--disable_latent_reasoning", action="store_true")
