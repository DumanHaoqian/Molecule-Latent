...
# Runtime config (edit here directly instead of CLI args)
PRECISION = '32'  # choices: 'bf16-mixed', '16', '32'
ENABLE_FLASH = False
PRETRAINED_MODEL_NAME_OR_PATH = 'DongkiKim/Mol-Llama-2-7B-Chat'
DEVICE = 0  # e.g. 0, 1, ... or 'cpu'
...
if __name__ == '__main__':
    ...
    llama_version = 'llama3' if 'Llama-3' in PRETRAINED_MODEL_NAME_OR_PATH else 'llama2'
    tokenizer = AutoTokenizer.from_pretrained(PRETRAINED_MODEL_NAME_OR_PATH)
    tokenizer.mol_token_id = tokenizer("<mol>", add_special_tokens=False).input_ids[0]
    
    model = MolLLaMA.from_pretrained(PRETRAINED_MODEL_NAME_OR_PATH, vocab_size=len(tokenizer),
                                    torch_dtype=torch_dtype, enable_flash=ENABLE_FLASH).to(device)
    # Load inputs
    input_list = json.load(open('playground_inputs.json', 'r'))

    text_batch = get_text_data(input_list, tokenizer, llama_version).to(device)
    smiles_list = []
    for input in input_list:
        if(input['smiles'] is  None):
            print("This is a caption to molecule task")
        else:
